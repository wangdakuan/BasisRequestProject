...
public final class ToastUtils {
    ...
    private static int yOffset = (int) (64 * AppUtils.getApp().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes final int layoutId) {
        LayoutInflater inflate = (LayoutInflater) AppUtils.getApp().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(final CharSequence text, final int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(AppUtils.getApp());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(AppUtils.getApp(), spannableString, duration);
            } else {
                sToast = Toast.makeText(AppUtils.getApp(), text, duration);
            }
        }
